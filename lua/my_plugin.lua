...
function M.unit_test_module_mockers()
  local parser = parsers.get_parser()
  local lang = parser:lang()
  local tree = parser:parse()[1]
  local root = tree:root()
  ...
  filename = filename:match("^(.*)%.%w+$") -- Remove the extension
  ...
  if curr_node == nil then
    p("No parent function_definition found on cursor position")
    return
  end

  local module_query = ts.query.parse(
    lang,
    [[ 
    (call function: 
      (attribute
  ...
  local module_func_query = ts.query.parse(
    lang,
    [[ 
    (call function: 
      (attribute
        attribute: (identifier) @module_function ))
    ]]
  )

  local modules = {}
  for _, match, _ in module_query:iter_captures(curr_node, 0) do
    table.insert(modules, ts.get_node_text(match, 0))
  end

  local module_funcs = {}
  for _, match, _ in module_func_query:iter_captures(curr_node, 0) do
    table.insert(module_funcs, ts.get_node_text(match, 0))
  end

  local module_mockers = ""
  for i, value in ipairs(modules) do
    module_mockers = module_mockers .. value .. " = module_mocker(" .. filename .. ', "' .. value .. '")\n'
  end

  local module_function_mockers = ""
  local module_function_assert_once = ""
  for i, function_name in ipairs(module_funcs) do
    local module_name = modules[i]

    module_function_mockers = module_function_mockers
      .. module_name
      .. "."
      .. function_name
      .. " = AsyncMock(return_value=MagicMock())\n"

    module_function_assert_once = module_function_assert_once
      .. module_name
      .. "."
      .. function_name
      .. ".assert_awaited_once()\n"
  ...
  local result = module_mockers .. "\n" .. module_function_mockers .. "\n" .. module_function_assert_once
...
vim.api.nvim_create_user_command("UnitTestModuleMockers", function()
  M.unit_test_module_mockers()
end, {})
